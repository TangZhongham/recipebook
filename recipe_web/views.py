...
def recipe_list(request):
    ...
    dietary_types = Recipe.objects.values_list(
        "dietary_restrictions", flat=True
    ).distinct()
...
def user_profile(request, pk):
    ...
    favorite_recipes = Recipe.objects.filter(favorites=profile_user).order_by(
        "-created_at"
    )
...
def toggle_follow(request, pk):
    ...
    return (
        redirect(next_url)
        if next_url
        else redirect("user_profile", pk=user_to_follow.pk)
    )
...
def followers_list(request, pk):
    ...
    return render(
        request,
        "recipe_web/followers.html",
        {"profile_user": user, "followers": followers},
    )
...
def following_list(request, pk):
    ...
    return render(
        request,
        "recipe_web/following.html",
        {"profile_user": user, "following": following},
    )
...
def feed(request):
    ...
    feed_recipes = Recipe.objects.filter(user__in=following_users).order_by(
        "-created_at"
    )
...
#                 email_message = render_to_string('password_reset_email.html', {'reset_url': reset_url, 'user': user})
